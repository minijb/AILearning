...
- [阶段三至六：Agent 智能体 + Skill 技能 + 斜杠命令与交互 + 工具详解](#阶段三至六agent-智能体--skill-技能--斜杠命令与交互--工具详解)
  ...
  - [第二章：Skill 技能系统](#第二章skill-技能系统)
    ...
    - [2.6 Plugins 插件系统详解](#26-plugins-插件系统详解)
...
> 详细说明参见 `tutorial-01-02-入门基础与核心功能.md` 第三章。以下是常用命令速查：

| 命令 | 用途 | 使用场景 |
|------|------|---------|
| `/model <name>` | 切换模型 | `opus`（复杂）/ `sonnet`（日常）/ `haiku`（快速） |
| `/clear` | 清除对话历史 | 开始新话题、重置上下文 |
| `/compact` | 压缩上下文 | 长任务中途、释放上下文空间 |
| `/debug` | 启用调试模式 | 诊断异常行为、排查问题 |
...
> 详细示例参见 `tutorial-01-02-入门基础与核心功能.md`。

**核心原则：** 具体 > 模糊，提供上下文和约束条件。

```
❌ "帮我看看代码"
✅ "检查 src/api/user.ts 中的错误处理逻辑，修复第 23 行的类型错误"
```
